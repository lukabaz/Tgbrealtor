...
async def parse_myhome(bot, loop):
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True)
        try:
            for chat_id in subscribed_users:
                filters = load_filters(chat_id)
                defaults = {'city': '2', 'deal_type': 'rent', 'price_from': 200, 'price_to': 1500, 'floor_from': 1, 'floor_to': 25,
                            'rooms_from': 1, 'rooms_to': 4, 'bedrooms_from': 1, 'bedrooms_to': 3, 'own_ads': '1'}
                for key, value in defaults.items():
                    filters.setdefault(key, value)

                pr_type = "1" if filters['deal_type'] == "rent" else "2"
                ...
                for attempt in range(3):
                    try:
                        ...
                        listings = soup.find_all('div', class_='card-wrapper')
                        logger.debug(f"Found {len(listings)} listings with 'card-wrapper'")

                        if not listings:
                            logger.info(f"No listings found for {chat_id} with filters: {filters}")
                            logger.debug(f"Page HTML snippet: {content[:1000]}")
                            await page.close()
                            break
...
def run_parser(bot, loop):
    ...
    while True:
        try:
            ...
            asyncio.run_coroutine_threadsafe(parse_myhome(bot, loop), loop).result()
        except Exception as e:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args and args[0].startswith("filters_"):
        ...
        filters = load_filters(chat_id)
        defaults = {
            'city': filters.get('city', '2'),
            'deal_type': filters.get('deal_type', 'rent'),
            'price_from': filters.get('price_from', 200),
            'price_to': filters.get('price_to', 1500),
            'floor_from': filters.get('floor_from', 1),
            'floor_to': filters.get('floor_to', 25),
            'rooms_from': filters.get('rooms_from', 1),
            'rooms_to': filters.get('rooms_to', 4),
            'bedrooms_from': filters.get('bedrooms_from', 1),
            'bedrooms_to': filters.get('bedrooms_to', 3),
        ...
        if chat_id not in subscribed_users:
            await update.message.reply_text("Привет! Я могу рассылать новые объявления.\n\nНажми 'Включить бот'.", reply_markup=None)
            return
...
async def enable_bot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Бот включен! Уведомления начнутся.\n\nНажми 'Открыть настройки':", reply_markup=get_settings_keyboard())
...
async def reset_seen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("Список объявлений сброшен.")
    except Exception as e:
...
async def handle_web_app_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        save_filters(chat_id, filters)
        logger.info(f"User {chat_id} updated filters via Web App: {filters}")
        ...
        logger.error(f"Error handling web app data for {chat_id}: {e}")
        await update.message.reply_text("Ошибка при обновлении фильтров.")
...
def main():
    try:
        ...
        application.run_webhook(listen="0.0.0.0", port=10000, url_path=TOKEN, webhook_url=webhook_url, allowed_updates=["message", "callback_query"])
    except Exception as e:
